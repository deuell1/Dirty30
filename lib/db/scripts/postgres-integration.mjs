...
async function scheduleWeekForeignKeyCheck() {
  ...
  try {
    const week = (
      await setup.query(
        `INSERT INTO schedule_weeks
         ...
         VALUES ($1, 91, '2026-06-01', '2026-06-01', '2026-06-07')
         RETURNING id`,
        [fixture.season.id],
      )
    ).rows[0];
    const game = (
      await setup.query(
        `INSERT INTO games
           (season_id, home_team_id, away_team_id, venue_id, court_id,
            scheduled_at, schedule_week, schedule_week_id, status)
         VALUES ($1, $2, $3, $4, $5, '2026-06-01T19:00:00Z', 91, $6, 'DRAFT')
         RETURNING id`,
        [
          fixture.season.id,
          fixture.home.id,
    ...
    const bye = (
      await setup.query(
        `INSERT INTO team_byes
           (season_id, team_id, schedule_week, schedule_week_id, play_date, source)
         VALUES ($1, $2, 91, $3, '2026-06-01', 'MANUAL')
         RETURNING id`,
        [fixture.season.id, fixture.home.id, week.id],
      )
    ).rows[0];
      ...
      `SELECT
         (SELECT count(*) FROM games AS g
          JOIN schedule_weeks AS sw
            ON sw.id = g.schedule_week_id AND sw.season_id = g.season_id
          WHERE g.id = $1) AS game_matches,
         (SELECT count(*) FROM team_byes AS b
          JOIN schedule_weeks AS sw
            ON sw.id = b.schedule_week_id AND sw.season_id = b.season_id
          WHERE b.id = $2) AS bye_matches`,
      [game.id, bye.id],
    );
    ...
    const otherSeason = (
      await setup.query(
        `INSERT INTO seasons
           (league_id, name, start_date, end_date, active)
         VALUES ($1, $2, '2026-01-01', '2026-12-31', false)
         RETURNING id`,
        [fixture.league.id, `${marker}-cross-season`],
      )
    ).rows[0];
    await setup.query("SAVEPOINT invalid_game");
    try {
      await assert.rejects(
        setup.query(
          `INSERT INTO games
             (season_id, home_team_id, away_team_id, venue_id, court_id,
              scheduled_at, schedule_week_id, status)
           VALUES ($1, $2, $3, $4, $5, '2026-06-01T20:30:00Z', $6, 'DRAFT')`,
          [
            otherSeason.id,
            fixture.home.id,
            fixture.away.id,
        ...
        "games must reject a schedule week from another season",
      );
    } finally {
      await setup.query("ROLLBACK TO SAVEPOINT invalid_game");
      await setup.query("RELEASE SAVEPOINT invalid_game");
    }
    await setup.query("SAVEPOINT invalid_bye");
    try {
      await assert.rejects(
        setup.query(
          `INSERT INTO team_byes
             (season_id, team_id, schedule_week, schedule_week_id, play_date, source)
           VALUES ($1, $2, 91, $3, '2026-06-01', 'MANUAL')`,
          [otherSeason.id, fixture.home.id, week.id],
        ),
        (error) => error?.code === "23503",
        "team_byes must reject a schedule week from another season",
      );
    } finally {
      await setup.query("ROLLBACK TO SAVEPOINT invalid_bye");
      await setup.query("RELEASE SAVEPOINT invalid_bye");
    }
  } finally {
...
async function main() {
  ...
  try {
    ...
    await scheduleWeekForeignKeyCheck();
    ...
    console.info(
      "PostgreSQL integration checks: 7 passed (migration default, schedule-week composite FKs, schedule week backfill, schedule concurrency, bye persistence, invitation acceptance, league initialization concurrency).",
    );
  } finally {
