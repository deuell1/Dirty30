...
async function seedUser(
  ...
  const existing = await db.query.users.findFirst({
    where: or(eq(users.phone, phone), eq(users.externalAuthId, externalAuthId)),
  });
  if (existing) {
...
async function seedMembership(
  ...
  if (!existing)
    await db.insert(teamMemberships).values({ teamId, userId, membershipRole });
}
...
async function seed() {
  const league = await firstOrCreate(
    ...
    async () =>
      (
        await db
          .insert(leagues)
          .values({ name: "Dirty 30 Beer League" })
          .returning()
      )[0]!,
  );
  ...
  const courtOne = await firstOrCreate(
    () =>
      db.query.courts.findFirst({
        where: and(eq(courts.venueId, venue.id), eq(courts.name, "Court 1")),
      }),
    async () =>
      (
        await db
  ...
  const courtTwo = await firstOrCreate(
    () =>
      db.query.courts.findFirst({
        where: and(eq(courts.venueId, venue.id), eq(courts.name, "Court 2")),
      }),
    async () =>
      (
        await db
          .insert(courts)
          .values({ venueId: venue.id, name: "Court 2" })
          .returning()
      )[0]!,
  );
    ...
    const team = await firstOrCreate(
      () =>
        db.query.teams.findFirst({
          where: and(eq(teams.seasonId, season.id), eq(teams.name, teamName)),
        }),
      async () =>
        (
          await db
  ...
  for (const [teamIndex, name] of [
    ...
    const [firstName, lastName] = name.split(" ");
    const player = await seedUser(
      `seed_player_${teamIndex}`,
      `+120255501${20 + teamIndex}`,
      firstName!,
      lastName!,
      "PLAYER",
    );
  ...
  const existingInvite = await db.query.playerInvitations.findFirst({
    where: and(
      eq(playerInvitations.teamId, hops.id),
      eq(playerInvitations.status, "PENDING"),
      or(
        eq(playerInvitations.invitedPhone, pendingPhone),
        eq(playerInvitations.tokenHash, seedInvitationTokenHash),
      ),
    ),
  });
  if (!existingInvite) {
    ...
    await db.insert(playerInvitations).values({
      ...
      tokenHash: seedInvitationTokenHash,
      expiresAt: new Date(Date.now() + 7 * 86400000),
    });
  } else if (existingInvite.invitedPhone !== pendingPhone) {
    await db
      .update(playerInvitations)
      .set({ invitedPhone: pendingPhone })
      .where(eq(playerInvitations.id, existingInvite.id));
  }
  ...
  for (const [
    ...
    const existing = await db.query.games.findFirst({
      where: and(
        ...
        eq(games.scheduledAt, values.scheduledAt),
      ),
    });
    if (existing)
  ...
  console.info(
    "Dirty-30 development seed is ready (Hops & Dreams has 8 occupied roster positions).",
  );
}
