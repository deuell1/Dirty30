...
const { getAuth, getUser, findUser, insertReturning } = vi.hoisted(() => ({
  getAuth: vi.fn(),
  getUser: vi.fn(),
  findUser: vi.fn(),
...
function response() {
  const res: {
    locals: { currentUser?: unknown };
    status: ReturnType<typeof vi.fn>;
    json: ReturnType<typeof vi.fn>;
  } = { locals: {}, status: vi.fn(), json: vi.fn() };
  res.status.mockReturnValue(res);
  return res;
}

describe("Clerk identity gate with isolated database mocks", () => {
  const originalBootstrapPhone = process.env.BOOTSTRAP_COMMISSIONER_PHONE;
  ...
  afterEach(() => {
    if (originalBootstrapPhone === undefined) {
      delete process.env.BOOTSTRAP_COMMISSIONER_PHONE;
    } else {
      process.env.BOOTSTRAP_COMMISSIONER_PHONE = originalBootstrapPhone;
    }
  });

  const verifiedClerkUser = (phoneNumber: string) => ({
    phoneNumbers: [
      { id: "phone_1", phoneNumber, verification: { status: "verified" } },
    ],
  ...
  it("rejects a Clerk user without a verified primary phone before database access", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      error: "A verified primary phone number is required",
    });
  });
  ...
  it("creates the verified bootstrap phone as an active commissioner", async () => {
    ...
    insertReturning.mockResolvedValueOnce([
      {
        id: 1,
        externalAuthId: "clerk_commissioner",
        phone: "+12025550100",
        role: "COMMISSIONER",
        accessState: "ACTIVE",
        active: true,
