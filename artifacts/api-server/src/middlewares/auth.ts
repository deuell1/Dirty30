...
export async function resolveCurrentUser(
  ...
  try {
    ...
    if (!primaryPhone || primaryPhone.verification?.status !== "verified") {
      return res
        .status(400)
        .json({ error: "A verified primary phone number is required" });
    }
    ...
    if (byExternalAuthId && byPhone && byExternalAuthId.id !== byPhone.id) {
      return res.status(409).json({
        error:
          "This verified phone number is already linked to a different league account",
      });
    }
    const bootstrapPhoneRaw = process.env.BOOTSTRAP_COMMISSIONER_PHONE?.trim();
    const bootstrapPhone = bootstrapPhoneRaw
      ? normalizeUsPhone(bootstrapPhoneRaw)
      : undefined;
    ...
    if (existing) {
      if (existing.phone !== normalizedPhone) {
        return res.status(403).json({
          error:
            "Your verified Clerk phone does not match this league account. Contact a commissioner before changing it.",
        });
      }
      const [updated] = await db
        ...
        .returning();
      if (!updated?.active || updated.accessState === "DISABLED")
        return res.status(403).json({ error: "League access is unavailable" });
    ...
    const [created] = await db
      ...
      .returning();
    if (!created?.active || created.accessState === "DISABLED")
      return res.status(403).json({ error: "League access is unavailable" });
...
export function requireCommissioner(
  ...
  if (
    res.locals.currentUser?.accessState !== "ACTIVE" ||
    res.locals.currentUser.role !== "COMMISSIONER"
  ) {
...
export function currentUser(req: Request, res: Response): User {
  ...
  if (!user)
    throw new Error(`Missing authenticated user for ${req.method} ${req.path}`);
  return user;
}
