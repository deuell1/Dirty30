...
router.patch("/scores/:gameId", requireCommissioner, async (req, res, next) => { try { const gameId = z.coerce.number().int().positive().parse(req.params.gameId); const input = SubmitScoreBody.parse(req.body); const game = await db.query.games.findFirst({ where: eq(games.id, gameId) }); if (!game) return res.status(404).json({ error: "Game not found" }); const actor = currentUser(req, res); await db.update(games).set({ ...input, status: "FINAL", resolvedByUserId: actor.id, resolvedAt: new Date() }).where(eq(games.id, gameId)); await audit(actor.id, "game", gameId, game.status === "FINAL" ? "SCORE_CORRECTED" : "COMMISSIONER_SCORE_ENTERED", game, input); return res.status(204).end(); } catch (error) { return next(error); } });
