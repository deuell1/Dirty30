...
export function schedulesConflict(
  ...
  const withinGameWindow =
    Math.abs(existing.startsAt.getTime() - proposed.startsAt.getTime()) <
    90 * 60_000;
  const sharesTeam = [existing.homeTeamId, existing.awayTeamId].some(
    (id) => id === proposed.homeTeamId || id === proposed.awayTeamId,
  );
  return (
    withinGameWindow && (sharesTeam || existing.courtId === proposed.courtId)
  );
}

export function nextScoreState(
  state: ScoreState,
  action: "SUBMIT" | "CONFIRM" | "DISPUTE" | "RESOLVE" | "CORRECT",
) {
  const transitions: Record<
    ScoreState,
    Partial<Record<typeof action, ScoreState>>
  > = {
    DRAFT: {},
...
export function auditActionForScore(
  state: ScoreState,
  action: "SUBMIT" | "CONFIRM" | "DISPUTE" | "RESOLVE" | "CORRECT",
) {
  if (!nextScoreState(state, action)) return undefined;
  return {
    SUBMIT: "SCORE_SUBMITTED",
    CONFIRM: "SCORE_CONFIRMED",
    DISPUTE: "SCORE_DISPUTED",
    RESOLVE: "SCORE_RESOLVED",
    CORRECT:
      state === "FINAL" ? "SCORE_CORRECTED" : "COMMISSIONER_SCORE_ENTERED",
  }[action];
}
