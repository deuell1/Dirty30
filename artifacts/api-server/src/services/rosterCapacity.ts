...
export class RosterCapacityError extends Error {
  ...
  constructor() {
    super(
      `Team roster is full. A team may have at most ${MAX_ROSTER_POSITIONS} occupied positions.`,
    );
  }
}

export function occupiedRosterPositions(
  ...
  await tx
    ...
    .where(
      and(
        ...
        sql`${playerInvitations.expiresAt} <= NOW()`,
      ),
    );
  return action();
}
...
export async function requireRosterSlot(tx: Transaction, teamId: number) {
  ...
  const occupied = occupiedRosterPositions(
    Number(memberCount[0]?.count ?? 0),
