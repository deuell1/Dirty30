...
describe("schedule and score acceptance policy", () => {
  it("blocks a shared team or court inside the 90-minute game window", () => {
    ...
    expect(
      schedulesConflict(existing, {
        startsAt: new Date("2026-08-25T20:00:00Z"),
        homeTeamId: 3,
        awayTeamId: 4,
        courtId: 10,
      }),
    ...
    expect(
      schedulesConflict(existing, {
        startsAt: new Date("2026-08-25T21:00:00Z"),
        homeTeamId: 3,
        awayTeamId: 4,
        courtId: 11,
      }),
  ...
  it("calculates standings only from final results", () => {
    ...
    expect(standings.find((row) => row.teamName === "Coral")).toMatchObject({
      played: 0,
      wins: 0,
    });
  });
