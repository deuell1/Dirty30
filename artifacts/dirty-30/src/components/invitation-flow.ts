...
type QueryCache = {
  ...
  refetchQueries: (options: { queryKey: readonly unknown[] }) => Promise<unknown>;
  getQueryData: <T>(queryKey: readonly unknown[]) => T | undefined;
};

export async function refreshAfterInvitationAcceptance(
  client: QueryCache,
  keys: { currentUser: readonly unknown[]; teams: readonly unknown[]; team: readonly unknown[]; roster: readonly unknown[] },
) {
  await Promise.all([
...
export function pendingSurface(pathname: string, accessState: UserProfile["accessState"]) {
  if (accessState !== "PENDING") return "active" as const;
  return pathname.startsWith("/invite/") ? "invitation" as const : "waiting" as const;
}