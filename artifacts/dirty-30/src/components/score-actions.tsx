...
function errorMessage(error: unknown) {
  if (typeof error === "object" && error) {
    ...
    return (
      value.data?.error ??
      value.message ??
      "The score change could not be saved."
    );
  }
...
export function ScoreActions({ game }: { game: Game }) {
  ...
  const ready =
    homeScore !== "" &&
    awayScore !== "" &&
    Number.isInteger(scoreInput.homeScore) &&
    Number.isInteger(scoreInput.awayScore) &&
    scoreInput.homeScore >= 0 &&
    scoreInput.awayScore >= 0;
  const refresh = () =>
    void Promise.all([
  ...
  if (game.status === GameStatus.CANCELLED)
    return (
      <StatusCard
        ...
        detail="No score can be recorded for a cancelled game."
      />
    );
  if (game.status === GameStatus.DISPUTED && !game.canManageScore)
    return (
      <StatusCard
        title="Awaiting commissioner resolution"
        detail={
          game.disputeReason
            ? `Dispute reason: ${game.disputeReason}`
            : "A captain disputed this score. The commissioner will resolve it."
        }
      />
    );
  if (game.status === GameStatus.FINAL && !game.canManageScore)
    return (
      <StatusCard
        title="Final score"
        detail="This verified result is now counted in the standings."
      />
    );

  return (
    <section
      className="mt-6 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-5 sm:p-6"
      data-testid="score-actions"
    >
      <p className="font-mono-custom text-[10px] font-bold uppercase tracking-[.18em] text-[hsl(var(--destructive))]">
        Game-day action
      </p>
      <h2 className="mt-1 font-display text-2xl font-bold">
        {game.status === GameStatus.PENDING_CONFIRMATION
          ? "Confirm or dispute score"
          : game.status === GameStatus.DISPUTED
            ? "Resolve disputed score"
      ...
      {game.status === GameStatus.PENDING_CONFIRMATION &&
      game.scoreSubmittedByCurrentUser &&
      !game.canManageScore ? (
        <StatusCard
          title="Awaiting opposing captain"
          detail="The other team's captain must confirm or dispute before this score reaches standings."
        />
      ) : null}
      {game.status === GameStatus.PENDING_CONFIRMATION &&
      game.canConfirmOrDisputeScore ? (
        <div className="mt-4 space-y-3">
          <p className="rounded-xl bg-[hsl(var(--muted)/.65)] p-3 text-sm">
            Submitted score:{" "}
            <strong>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              onClick={() =>
                dispute.mutate(
                  { gameId: game.id, data: { reason: reason.trim() } },
                  { onSuccess: refresh },
                )
              }
      ...
      {(game.canSubmitScore ||
        (game.status === GameStatus.DISPUTED && game.canManageScore) ||
        (game.canManageScore &&
          (game.status === GameStatus.FINAL ||
        ...
        <form onSubmit={save} className="mt-4">
          ...
          {confirmCorrection && (
            <p className="mt-3 rounded-xl bg-[hsl(var(--accent)/.24)] p-3 text-sm font-semibold">
              Confirm this final-score correction. The before and after values
      ...
      {game.status === GameStatus.DISPUTED &&
        game.canManageScore &&
        game.disputeReason && (
          <p className="mt-4 rounded-xl bg-[hsl(var(--accent)/.18)] p-3 text-sm">
            Dispute reason: {game.disputeReason}
          </p>
        )}
      <p className="mt-4 text-xs text-[hsl(var(--muted-foreground))]">
        Every submitted, confirmed, disputed, resolved, and corrected score is
        recorded in the league audit trail.
      </p>
    </section>
  );
}
...
function StatusCard({ title, detail }: { title: string; detail: string }) {
  return (
    <div className="mt-4 rounded-xl bg-[hsl(var(--muted)/.65)] p-4">
