    ...
    <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
      <header className="sticky top-0 z-10 border-b border-[hsl(var(--border))] bg-[hsl(var(--card))] px-5 py-4">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-4">
          ...
          <nav className="flex flex-1 flex-wrap gap-1">
            {navItems.map(([href, label]) => (
              <Link
                key={href}
                href={href}
                className={`rounded-lg px-3 py-2 text-sm font-bold ${location === href || (href !== "/" && location.startsWith(href)) ? "bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]" : "text-[hsl(var(--muted-foreground))]"}`}
              >
                {label}
              </Link>
            ))}
            {user?.role === DashboardRole.COMMISSIONER && (
              <Link
                href="/review"
                className="rounded-lg px-3 py-2 text-sm font-bold text-[hsl(var(--muted-foreground))]"
              >
          ...
          <Link href="/settings" className="text-sm font-bold">
            Profile
          </Link>
          <button
            onClick={() => void signOut()}
            className="text-sm font-bold text-[hsl(var(--muted-foreground))]"
          >
            Sign out
          </button>
        </div>
    ...
    <section>
      <div className="mb-6 flex items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-4xl font-extrabold">Teams</h1>
        </div>
        {isCommissioner && (
          <form
            onSubmit={(event) => {
...
function Standings() {
  ...
  return (
    <section>
      ...
      <h1 className="mb-6 font-display text-4xl font-extrabold">Standings</h1>
      <div className="overflow-hidden rounded-2xl border border-[hsl(var(--border))]">
        {(standings.data as Standing[] | undefined)?.map((row) => (
          <div
            key={row.teamName}
            className="grid grid-cols-[36px_1fr_repeat(3,auto)] gap-4 border-b p-4 last:border-0"
          >
            <b>{row.rank}</b>
            <b>{row.teamName}</b>
...
function ReviewPersisted() {
  ...
  return (
    <section>
      ...
      <h1 className="mb-6 font-display text-4xl font-extrabold">
        Review queue
      </h1>
